...
실행 순서를 임의로 조절하기 위하여 각 함수 호출문에 await 키워드를 부착한다
*/
const std_num_fetchV2 = async (st_num) => {
...
const std_num_fetch = (st_num) => {
  ...
  // fetch 가 완료되기 전에 실행되어 st_num_fetch
  // 주석 차후
};

// save_cb 함수를 Promis 방식으로 변경
const save_cb = async () => {
  ...
  // st_num_yes 에 true 나 false 가 담길것으로 기대
  //const st_num_yes = st_num_fetchV2(st_num.value);
  // std_num_fetchV2(st_num.value).then((st_num_yes) => {
  //   console.log("st_num_yes : ", st_num_yes);

  //   // false 가 담기면 학번 항목을 clear 하고 focus 를 담는다
  //   // 중복된 학번일 경우가 해당된다
  //   if (!st_num_yes) {
  //     st_num.value = "";
  //     st_num.focus();
  ...
  //   }
  // });
  // 서버에 학번 중복 검사를 의뢰하고 기다리기
  const st_num_yes = await std_num_fetchV2(st_num.value);
  console.log("st_num_yes", st_num_yes);
  if (!st_num_yes) {
    alert("이미 등록된 학번입니다\n 다시 확인하세요");
    st_num.focus();
  ...
  /* Number() 함수
    ...
    숫자형이 아닌 다른 데이터가 포함되어 숫자로 바꿀 수 없을경우
    java 에서는 MumberFormatException 이 발생하고 코드가 중단된다
    코드 중단을 방지하기 위하여 try-catch 로 처리를 해주어야 한다
  
    그런데 JS 에서는 그러한 상황에 Exception 이 발생하는 대신
    NaN(not a number) 이라는 값을 return 한다
  
    JS 에서 사용되는 특별한 데이터들이 있다
    NaN, undefinded(정의되지 않은 변수)
    null, 0 등등 몇몇의 데이터는 특별하게 사용된다
    변수에 이러한 값들이 저장되어 있을때 
    if(변수) 명령문에서는 false 가 된다
    */
