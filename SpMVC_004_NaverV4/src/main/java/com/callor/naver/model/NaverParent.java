...
public class NaverParent<VO> {
	...
	/*
	 * Naver API 를 사용하여 News, Books, Movie 정보를 가져오려고 한다
	 * 이때 RestTemplate 를 사용하여 데이터를 가져오는데
	 * RestTemplate 은 VO 를 감싸는 List 변수가 한개 필요하다
	 * 그런데 List 를 감싸는 VO 가 News, Books, Movie 들이 각각 다르다
	 * 이럴때 parent 클래스를 각각의 VO를 감싸는 타입으로 복수 정의 해야하는 문제가 있다
	 * 
	 * 이때도 Generic 의 기능을 사용하여 
	 * 클래스 선언문에 Generic 을 정의해 두고
	 * 이 클래스를 사용할 때 확정된 VO 를 부착하여 사용한다
	 */
