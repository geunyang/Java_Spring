document.addEventListener("DOMContentLoaded", () => {
  ...
  // 정규식 문법
  // 문자열 중에서 tag 문자열을 무조건 삭제(제거)하기위한 패턴
  ...
  /*
      input event
      input tag 에 키보드로 문자열을 입력할 때마다 발생하는 event
      */
  // if(book_isbn)을 실행하여 book_isbn 이 있으면 addEventListner 를 부착하고
  ...
  modal_box.addEventListener("click", (tb) => {
    ...
    const td = tb.target;
    if (td.tagName === "TD") {
      // td 를 감싸고 있는 가장 가까운 영역의 tr 추출하기
  ...
  btn_save?.addEventListener("click", (btn) => {
    const isbn = inputs[input_index.image];
    ...
    // click evevt 가 발생한 tag 에서 가장 가까운 form 을 추출하기
