...
<script src="${rootPath}/static/js/login.js?20220609003"></script>
</head>
<body>
	<form method="POST" class="w3-container w3-card-4 login">
		...
		<c:if test="${error == 'USERNAME_FAIL'}">
			<div class="w3-red w3-padding-16">존재하지 않는 ID 입니다</div>
		</c:if>
		<c:if test="${error == 'PASSWORD_FAIL'}">
			<div class="w3-red w3-padding-16">비밀번호가 틀립니다</div>
		</c:if>
		...
		<button class="w3-button w3-green btn_login" type="button">로그인</button>
	</form>
</body>
</html>