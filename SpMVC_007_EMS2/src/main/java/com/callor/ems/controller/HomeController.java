...
public class HomeController {
	...
	public String home(@ModelAttribute("emsVO") EmsVO emsVO, Model model) {
		...
		//naver_username에 괄호가 있는 문자열이 있는가
		if(matcher.find()) {
	 ...
	 * BindingResult 객체에 오류 메세지가 포함된다
	 ...
	 * 
	 * 보통 파일 업로드를 받을 때는 파일 경로를 수신할 매개변수에
	 * @RequestParam() 을 사용한다
	 * 하지만 여러 파일을 수신하는
	 * MultipartHttpServletRequest 를 매개변수로 사용할때는
	...
	/*
	 * @RequestParam()
	 * 1. client 에서 보내는 변수와 실제 사용하는 변수의 이름이  다를경우
	 * 2. 매개변수가 숫자형 일때 client 에서 변수에 값을 담지 않고 요청을 하면
	 * 	내부 오류가 발생한다. 이떄 required 를 false 로 선언하고 
	 * 	defaultValue 를 설정하면 오류를 일부 방지할 수 있다
	 */
	@ResponseBody
	@RequestMapping(value = "/num", method=RequestMethod.GET)
	public String nums(@RequestParam(name="num",required = false, defaultValue = "0") Integer numValue) {
