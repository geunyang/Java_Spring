...
public class StudentController {
	 ...
	 * 요청받는 method 에 HttpSession 객체를 매개변수로 설정하고
	 * 코드 시작부분에서 httpSession 의 getAttribute("변수명") 을 사용하여
	 * 로그인 된 정보를 추출한다
	 * 만약 추출한 정보가 null 이면 로그인 페이지로 점프하고
	 * null 이 아니면 요청에 대한 응답을 실행한다
	 */
//	@ResponseBody
	 ...
	 * 문자열을 web client 에게 그대로 전송하는 역하을 하도록
	 * 지시하는 Annotation 이다
	 * pom.xml 에 jackson-bind Dependency 를 추가하면
	 * 
	 * List, 객체(VO) 등등의 데이터를 json type 으로
	 * 변환하여 web client 로 전송한다
	 */
	@ResponseBody
